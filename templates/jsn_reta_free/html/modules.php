...
function modChrome_jsnmodule( $module, &$params, &$attribs ) {
	if ($module->content) {
		...
		if (isset( $attribs['class'] ))
		{
			...
			if (preg_match("/\bjsn-line-title\b/", (string) $attribs['class'])) {
				$firsttitle = explode(' ',trim($module->title));
				$moduleTitleOuput = '<span><span class="jsn-firstword">'.$firsttitle[0].'</span>'.trim(str_replace($firsttitle[0], '', $module->title)).'</span>';
			}
