		...
		<li class="<?php echo ($key%2) ? "odd" : "even"; if(count($comments)==$key+1) echo ' lastItem'; ?>">
			...
			<a href="<?php echo $comment->link; ?>">
				<?php if($params->get('itemTitle')): ?>
					<span class="lcItemTitle"><?php echo $comment->title; ?></span>
				<?php endif; ?>
			...
			<div class="jsn-k2-cus">
				<a class="k2Avatar lcAvatar" href="<?php echo $comment->link; ?>" title="<?php echo K2HelperUtilities::cleanHtml($comment->commentText); ?>"> <img src="<?php echo $comment->userImage; ?>" alt="<?php echo JFilterOutput::cleanText($comment->userName); ?>" style="width:<?php echo $lcAvatarWidth; ?>px;height:<?php echo $lcAvatarWidth; ?>px;" /> </a>
				<?php endif; ?>
				<?php if($params->get('commenterName')): ?>
				<div class="jsn-k2-cus-ct">
					<p class="lcUsername"><?php //echo JText::_('K2_WRITTEN_BY'); ?>
					...
					<p class="lcCommentDate"><i class="jsn-icon-calendar"></i>
						<?php if($params->get('commentDateFormat') == 'relative'): ?>
						<?php echo $comment->commentDate; ?>
						<?php else: ?>
						<?php echo JHTML::_('date', $comment->commentDate, JText::_('DATE_FORMAT_LC')); ?>
						<?php endif; ?>
					</p>
					<?php endif; ?>
				</div>
